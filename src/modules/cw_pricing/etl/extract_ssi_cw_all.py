# -*- coding: utf-8 -*-
"""
🏆 FINVISTA: BATCH COVERED WARRANT HISTORICAL DATA EXTRACTOR
===========================================================
Fetches all active covered warrant symbols, downloads their entire historical price 
series from vnstock, sorts the records, and exports both individual and consolidated CSVs.
Guarantees 100% completion via multi-pass scanning and rate-limit cool-down periods.

Usage:
  python scripts/run_cw_history_all.py --days 180 --compile
...
def main():
    parser = argparse.ArgumentParser(description="Fetch, sort and export historical prices for all active covered warrants")
    parser.add_argument('--days', '-d', type=int, default=180,
                        help="Number of calendar days of history to fetch (default: 180)")
    ...
    parser.add_argument('--compile', '-c', action='store_true',
                        help="Compile all historical prices into a single master CSV file")
    parser.add_argument('--compile-file', type=str, default=os.path.join("data", "processed", "all_cw_historical_prices.csv"),
                        help="Path to save the master compiled CSV file")
    parser.add_argument('--limit', '-l', type=int, default=None,
                        help="Limit the number of warrants to fetch (default: None, fetch all)")
    parser.add_argument('--force-download', '-f', action='store_true',
                        help="Force download even if the CSV file exists (ignores cache)")
    args = parser.parse_args()

    print("=" * 80)
    print(" 🚀 FINVISTA BATCH COVERED WARRANT HISTORY EXTRACTOR")
    print(f"  Looking back: {args.days} days | Output directory: {args.output_dir}")
    if args.limit:
        print(f"  Limit: {args.limit} warrants")
    print("=" * 80)

    # 1. Fetch live market list to get all active warrant symbols
    try:
        cw_df = fetch_market_cw_data()
    except Exception as e:
        print(f"❌ Failed to fetch current covered warrant list: {e}")
        return

    if cw_df.empty:
        print("❌ No covered warrants found in the market scan.")
        return

    # Sort warrant symbols alphabetically to process them in order
    active_symbols = sorted(cw_df['A_MaCW'].dropna().unique().tolist())
    if args.limit:
        active_symbols = active_symbols[:args.limit]
    print(f"🎯 Target list: {len(active_symbols)} active covered warrants.")
    ...
    # Multi-pass setup
    pending_symbols = list(active_symbols)
    max_passes = 5
    pass_num = 1
    success_count = 0
    ...
    try:
        while pending_symbols and pass_num <= max_passes:
            if pass_num > 1:
                print(f"\n⚠️  [PASS {pass_num}/{max_passes}] Retrying for {len(pending_symbols)} failed/skipped warrants...")
            ...
            for i, symbol in enumerate(pending_symbols, 1):
                ...
                if not args.force_download and os.path.exists(out_file) and os.path.getsize(out_file) > 0:
                    success_count += 1
                    # Less verbose output for cached files on subsequent passes
                    if pass_num == 1:
                ...
                for attempt in range(1, max_retries + 1):
                    try:
                        ...
                        if hist.empty or 'close' not in hist.columns:
                            print(" ⚠️ Empty data (newly listed/expired). Saved empty file to cache.")
                            # Save an empty CSV with headers to mark as scanned, avoiding future API calls
                        ...
                        # Sort individual warrant history chronologically by date
                        ...
                        if attempt < max_retries:
                            ...
                            # Do not crash; record as failed for this pass
                            pass
        ...
        if args.compile:
            print(f"🗂️ Compiling all available historical data from {args.output_dir}...")
            all_warrants_data = []
            ...
            for symbol in active_symbols:
                ...
                if os.path.exists(out_file) and os.path.getsize(out_file) > 0:
                    try:
                        df_warrant = pd.read_csv(out_file)
                        if not df_warrant.empty:
                            all_warrants_data.append(df_warrant)
                    except Exception as e:
                        print(f"⚠️  Could not read cached file for {symbol}: {e}")

            if all_warrants_data:
                compiled_df = pd.concat(all_warrants_data, ignore_index=True)
